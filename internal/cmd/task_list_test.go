...
func Test_runList(t *testing.T) {
	...
	t.Run("filter by single label", func(t *testing.T) {
		is := is.NewRelaxed(t)
		output, err := exec(t, "list", runList, "list", "-l", "first", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
	...
	t.Run("filter by multiple labels", func(t *testing.T) {
		is := is.NewRelaxed(t)
		output, err := exec(t, "list", runList, "list", "-l", "first,second,third,feature", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
	...
	t.Run("filter by status", func(t *testing.T) {
		is := is.NewRelaxed(t)
		output, err := exec(t, "list", runList, "list", "-s", "todo", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
	...
	t.Run("filter by multiple statuses", func(t *testing.T) {
		is := is.NewRelaxed(t)
		output, err := exec(t, "list", runList, "list", "-s", "todo,in-progress", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
	...
	t.Run("filter by assigned user", func(t *testing.T) {
		is := is.New(t)
		//
		output, err := exec(t, "list", runList, "list", "-a", "first-user", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), 1)
	...
	t.Run("filter by multiple assigned users", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "-a", "first-user,second-user", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
	...
	t.Run("filter by priority", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "--priority", "high", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), 1)
	...
	t.Run("sort by title", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "--sort", "title", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), countTask)
	...
	t.Run("sort by title reversed", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "--sort", "title", "--reverse", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), countTask)
	...
	t.Run("sort by priority", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "--sort", "priority", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), countTask)
	...
	t.Run("multiple sort fields", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "--sort", "priority,title", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), countTask)
	...
	t.Run("markdown output format", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "-m")
		is.NoErr(err)
	...
	t.Run("hide extra fields", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "--hide-extra")
		is.NoErr(err)
		outputStr := string(output)
	...
	t.Run("combined filters", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "-l", "second", "-s", "todo", "--priority", "medium", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
	...
	t.Run("empty results", func(t *testing.T) {
		is := is.NewRelaxed(t)
		output, err := exec(t, "list", runList, "list", "-l", "nonexistent", "-j")
		is.NoErr(err)
	...
	t.Run("empty results table format", func(t *testing.T) {
		is := is.NewRelaxed(t)
		output, err := exec(t, "list", runList, "list", "-l", "nonexistent")
		is.NoErr(err)
		outputStr := strings.TrimSpace(string(output))
	...
	t.Run("filter unassigned tasks", func(t *testing.T) {
		is := is.NewRelaxed(t)
		output, err := exec(t, "list", runList, "list", "--unassigned", "-j")
		is.NoErr(err)
	...
	t.Run("short flag aliases", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), countTask)
		...
		output, err = exec(t, "list", runList, "list", "-e")
		is.NoErr(err)
		outputStr = string(output)
		is.True(!strings.Contains(outputStr, "LABELS"))

		// Test short flag for reverse order
		output, err = exec(t, "list", runList, "list", "-r", "-j")
		is.NoErr(err)
		tasks = []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), countTask)
		...
		is.NoErr(err) // Should not error, may return empty array
		tasks = []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
		is.Equal(len(tasks), 1)
	...
	t.Run("invalid sort field", func(t *testing.T) {
		is := is.New(t)
		output, err := exec(t, "list", runList, "list", "--sort", "invalidfield", "-j")
	...
	t.Run("empty sort field", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "--sort", "", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
	...
	t.Run("multiple combined flags", func(t *testing.T) {
		...
		output, err := exec(t, "list", runList, "list", "-l", "second", "-s", "todo", "--sort", "title", "-r", "--hide-extra", "-j")
		is.NoErr(err)
		tasks := []*core.Task{}
		is.NoErr(json.Unmarshal(output, &tasks))
