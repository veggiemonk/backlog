...
func (f *FileTaskStore) Archive(id TaskID) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("could not get task %q: %w", id, err)
	}
	...
	if err := f.fs.MkdirAll(archivedDir, 0750); err != nil {
		return "", fmt.Errorf("could not create archived directory: %w", err)
	}
	...
	if err := f.fs.Rename(oldPath, newPath); err != nil {
		return "", fmt.Errorf("could not move task file: %w", err)
	}
